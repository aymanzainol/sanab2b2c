...
def analyze_readiness(row, checklist_cols):
    ...
    for col in checklist_cols:
        ...
        # تم توسيع الكلمات الإيجابية والسلبية لالتقاط جميع إجابات فريق العمل
        elif any(p in val for p in ['نعم', 'مطابق', 'مكتمل', 'تم', 'يوجد', 'متوفر', 'جاهز']): 
            current_score = 100.0
        elif any(n in val for n in ['لا', 'غير', 'لم', 'ناقص']): 
            current_score = 0.0
...
def load_data():
    ...
    # 1. استخراج الـ Unified_ID أولاً وتنظيفه من أي مسافات زائدة
    df['Unified_ID'] = np.where(df['شركة'].str.contains('ركين', na=False), df.iloc[:, 5], df.iloc[:, 6])
    # مسح المسافات الزائدة (Strip) لضمان عدم تكرار الخيمة إذا أضاف الفريق مسافة بالخطأ
    df['Unified_ID'] = df['Unified_ID'].fillna("غير معرف").astype(str).str.strip() 
    
    # 2. ترتيب البيانات حسب وقت التسجيل (Timestamp) لضمان أخذ التحديث الأخير
    if 'طابع زمني' in df.columns:
        df['طابع زمني'] = pd.to_datetime(df['طابع زمني'], errors='coerce')
        df = df.sort_values(by='طابع زمني', ascending=True)
        
    # 3. إزالة التكرارات والإبقاء على آخر تحديث فعلي
    df = df.drop_duplicates(subset=['Unified_ID'], keep='last')
    
    # 4. حساب الجاهزية للصفوف المتبقية
...
try:
    ...
    if page == "📊 الإحصائيات":
        ...
        # تجنب الخطأ في حال كانت البيانات فارغة
        sana_avg = round(df_sana['Overall_Score'].mean()) if not df_sana.empty else 0
        c1.metric("متوسط الإنجاز", f"{sana_avg}%")
        
        if not df_sana.empty:
            ...
            fig_sana.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", yaxis=dict(range=[0, 100]))
            c2.plotly_chart(fig_sana, use_container_width=True)

        # عرض ركين
        st.subheader("🟤 شركة ركين")
        r1, r2 = st.columns([1, 4])
        rakeen_avg = round(df_rakeen['Overall_Score'].mean()) if not df_rakeen.empty else 0
        r1.metric("متوسط الإنجاز", f"{rakeen_avg}%")
        
        if not df_rakeen.empty:
            ...
            fig_rakeen.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", yaxis=dict(range=[0, 100]))
